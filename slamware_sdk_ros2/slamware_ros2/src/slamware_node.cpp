...
SlamwareNode::SlamwareNode() : Node("slamware_node")
{
  ...
  auto qos_ = rclcpp::QoS(rclcpp::QoSInitialization(qos_profile_.history, 0), qos_profile_);
  
  rpos_platform = rpos::robot_platforms::SlamwareCorePlatform::connect("192.168.11.1", 1445);
  ...
  // map_pub_        = this->create_publisher<nav_msgs::msg::OccupancyGrid>("map", qos_);
  ...
  //timer_ = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&SlamwareNode::publish_laser_scan, this));
}
...
void SlamwareNode::publish_laser_scan()
{
  ...
  scan_msg_->header.frame_id = "base_scan"; // odom (it will be changed to base_scan) 
...
void SlamwareNode::broadcastMap2Laser(){
  ...
  odom_msg_->header.frame_id = "odom"; // odom
  odom_msg_->child_frame_id = "base_footprint"; // base_link
